  ...
  heroLoad = import("./candle3d")
    .then(({ mountCandle3D }) => {
      if (candleMount.isConnected && !disposeHero) {
        return mountCandle3D(candleMount).then((dispose) => {
          if (candleMount.isConnected && !disposeHero) {
            disposeHero = dispose;
          } else {
            dispose();
  ...
  ritualLoad = import("./ritual3d")
    .then(({ mountRitual3D }) => {
      if (ritual3dMount.isConnected && ritual.isConnected && !ritual.classList.contains("lit")) {
        return mountRitual3D(ritual3dMount, enterSite).then((controller) => {
          if (ritual3dMount.isConnected && ritual.isConnected && !ritual.classList.contains("lit")) {
            ritual3d = controller;
          } else {
            controller.dispose();
...
const startBurnCandle = () => {
  ...
  burnLoad = import("./burnCandle3d")
    .then(({ mountBurnCandle3D }) => {
      ...
      return mountBurnCandle3D(burnCandleMount).then((controller) => {
        if (!burnCandleMount.isConnected) {
          controller.dispose();
          return;
        }
